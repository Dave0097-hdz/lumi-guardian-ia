...
class UFWManager:
    ...
    def _ejecutar(self, argumentos):
        ...
        try:
            ...
            logger.error("Timeout de %ds ejecutando UFW", TIMEOUT_SEG)
    ...
    def _snapshot(self):
        ...
        if "Status: active" not in salida:
            logger.critical(
                "UFW esta INACTIVO. Las reglas no filtran trafico. "
                "Ejecutar 'ufw enable'."
            )
    ...
    def desbloquear(self, ip_str, motivo=""):
        red = self._parsear(ip_str)
        if red is None:
